class Page {
  ...
  init (paramsObj) {
    ...
    // 页面元素的外部容器
    state.container = paramsObj.container || document.body
    ...
    state.goCName = paramsObj.goCName || 'page-go'
    state.goNumCName = paramsObj.goNumCName || 'page-go-number'
    // 触发切换页面的事件
  ...
  switchPage () {
    ...
    let go = this.selectorEle('.' + state.goCName);
    let goNumber = this.selectorEle('.' + state.goNumCName);
    go.addEventListener('click',()=> {
      let pageNum = goNumber.value || 0
  ...
  renderPageDOM () {
    // 渲染页码DOM
    let state = this.state
    let pageContainer =  state.container
    ...
    paginationStr += `
    <span>
        <input type="text" class="page-go-number"   />
        <input value="Go" class="page-go" type='button' />
    </span>`
