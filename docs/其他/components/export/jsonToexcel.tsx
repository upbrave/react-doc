...
export default () => {
  ...
  useEffect(() => {
    const xlsx =
      'https://cdn.bootcdn.net/ajax/libs/xlsx/0.17.0/xlsx.core.min.js';
    const fileSaver =
      'https://cdn.bootcdn.net/ajax/libs/FileSaver.js/2.0.5/FileSaver.min.js';

    Promise.all([loadScript(xlsx), loadScript(fileSaver)]).then(() => {
      setLoad(true);
    });
  }, []);
  ...
  return (
    <div>
      <pre>{aoa.map((item) => JSON.stringify(item)).join('\n')}</pre>
